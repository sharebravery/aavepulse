...
export function MetricCard({ label, value, detail, icon, tone = 'blue', featured = false }: { label: string; value: string; detail: string; icon: ReactNode; tone?: 'blue' | 'cyan' | 'mint' | 'amber'; featured?: boolean }) {
  ...
  return (
    <article className={cn('panel-sheen rounded-card border border-line bg-surface p-5 transition-colors hover:border-line-active', featured && 'md:col-span-2 bg-gradient-to-br from-blue/[0.14] via-surface to-surface')}>
