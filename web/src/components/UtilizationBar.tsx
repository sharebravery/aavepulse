...
export function UtilizationBar({ value, showValue = false, className }: { value: number | string; showValue?: boolean; className?: string }) {
  ...
  return (
    <div className={cn('flex min-w-28 items-center gap-2', className)}>
      <div role="progressbar" aria-label="资金利用率" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percentage} className="h-1.5 min-w-12 flex-1 overflow-hidden rounded-full bg-white/[0.08]">
